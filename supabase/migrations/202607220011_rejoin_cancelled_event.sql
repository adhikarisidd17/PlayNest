...
begin
  ...
  if v_accepted+v_child_count>v_event.max_children then v_status:='waitlisted';
  elsif v_event.approval_mode='automatic' then v_status:='accepted';else v_status:='pending';end if;
  if v_request_id is null then
    insert into public.event_requests(event_id,requester_id,status,child_count,adult_count,message)
    values(p_event_id,auth.uid(),v_status,v_child_count,p_adult_count,nullif(trim(p_message),'')) returning id into v_request_id;
  else
